...
class BaseReal:
    ...
    def start_recording(self):
        """开始录制视频"""
        # Recording functionality removed: no-op
        self.recording = False
        return
    ...
    def process_frames(self,quit_event,loop=None,audio_track=None,video_track=None):
        ...
        if self.opt.transport=='virtualcam':
            audio_thread.join()
            vircam.close()
        else:  # webrtc: send explicit EOS to both tracks to terminate cleanly
            try:
                asyncio.run_coroutine_threadsafe(video_track._queue.put((None, 'eof_video')), loop)
            except Exception:
                pass
            try:
                asyncio.run_coroutine_threadsafe(audio_track._queue.put((None, 'eof_audio')), loop)
            except Exception:
                pass
