...
class SyncClock:
    """轻量级同步时钟：只在首帧对齐音/视频的起始时间，不做后续复杂同步。"""
    ...
    async def mark_ready_and_wait_start(self, kind: str) -> float:
        ...
        # 如果已经被另一条轨道启动了，直接复用那个时间
        return self._start_time if self._start_time is not None else time.time()


class PlayerStreamTrack(MediaStreamTrack):
    """
    A video/audio track that pulls frames from the player's queue and
    assigns monotonically increasing timestamps with a fixed packetization
    interval (AUDIO_PTIME / VIDEO_PTIME).
    """
